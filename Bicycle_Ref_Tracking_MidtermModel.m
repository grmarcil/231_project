...
for k = 1:N
    ...
    constr = [constr; x{k+1}(3)==x{k}(3) + u{k}(2)];
      ...
      constru = [constru; u{k}(1)>=L/tan(delmax)];
...
%   
         ...
         cost = norm(ur)^2;
% 
%        cost = norm(ur)^2 + 5*(norm(term1)^2+norm(term2)^2+norm(term3)^2);
%     
      
%       
%              
constr1 = future_r(1,(2:end))-future_r(1,(1:end-1))+ur(1,:).*sin(future_r(3,(1:end-1)))-ur(1,:).*sin(future_r(3,1:end-1)+ur(2,:)) ==0;
constr1 = [constr1, future_r(2,(2:end))-future_r(2,(1:end-1))-ur(1,:).*cos(future_r(3,(1:end-1)))+ur(1,:).*cos(future_r(3,1:end-1)+ur(2,:))==0];
constr1 = [constr1, future_r(3,(2:end))-future_r(3,(1:end-1))-ur(2,:)==0];
      


        constr3 = [ur(2,:)<=pi; -ur(2,:)<=pi];
        constr3 = [constr3; ur(1,:)>=L/tan(delmax)];
...
%     constr13 = []; 
    ...
    options = sdpsettings('solver','fmincon','verbose',1,'usex',1);
