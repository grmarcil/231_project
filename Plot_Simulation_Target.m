...
% ------------------------------------------------------------------------------
% Plot_Simulation_Target(z_cl, z_ref, z_targ, u_cl, lane_width, lane_length, path)
% ------------------------------------------------------------------------------
...
% z_ref: state reference trajectory of ego vehicle
% z_ref_targ: state reference trajectory of target vehicle
...
function animation = Plot_Simulation_Target(z_cl, u_cl, z_targ, lane_width, lane_length, path)
...
% intersection
    ...
    for i=1:length(z_cl)
        ego = Plot_Car(z_cl(:,i), u_cl(:,i), auto, fig, [0.7 0.7 1]);
        % target vehicle just goes straight for now, could log u later
        target = Plot_Car(z_targ(:,i), [0;0], auto, fig, [0.9 0.9 0.9]);
